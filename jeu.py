...
def main_menu(screen, width, height):
    font = pygame.font.SysFont(None, 48)
    menu_text = font.render("Choisissez le mode de jeu", True, (255, 255, 255))
    screen.fill((0, 0, 0))
    screen.blit(menu_text, (width // 2 - menu_text.get_width() // 2, height // 4))

    player_vs_player_button = pygame.Rect(width // 4, height // 2, width // 2, 50)
    player_vs_ai_button = pygame.Rect(width // 4, height // 2 + 100, width // 2, 50)

    pygame.draw.rect(screen, (255, 0, 0), player_vs_player_button)
    pygame.draw.rect(screen, (0, 0, 255), player_vs_ai_button)

    pvp_text = font.render("PVP", True, (255, 255, 255))
    ai_text = font.render("IA", True, (255, 255, 255))

    screen.blit(pvp_text, (width // 2 - pvp_text.get_width() // 2, height // 2 + 10))
    screen.blit(ai_text, (width // 2 - ai_text.get_width() // 2, height // 2 + 110))
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if player_vs_player_button.collidepoint(event.pos):
                    lancerJeu(IA=False)
                elif player_vs_ai_button.collidepoint(event.pos):
                    lancerJeu(IA=True)
def lancerJeu(IA):                  
